...
- **In `Cargo.toml` on `dev`**: `0.4.2-alpha`. `v0.4.0-alpha` failed publish at the Trivy HIGH gate (libpcre2 CVEs from `debian:bookworm-slim`). `v0.4.1-alpha` was the fix — musl-static + distroless/static via per-arch `FROM builder-${TARGETARCH}` — but ALSO failed publish: BuildKit refuses to interpolate `TARGETARCH` in a stage NAME at graph-solve time. `0.4.2-alpha` rewrites the multi-arch build around `cargo-zigbuild` (single builder, TARGETARCH dispatch inside a RUN step) — verified locally to produce a 0-CVE image. Post-release: two container tags need manual cleanup — `:0.4.0-alpha` (unsigned, vulnerable) exists on both registries; `:0.4.1-alpha` was NEVER pushed to any registry (build failed before push), so no cleanup needed there.
