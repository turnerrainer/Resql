...
- **In `Cargo.toml` on `dev`**: `0.4.0-alpha`. The `v0.4.0-alpha` tag was pushed 2026-09-13 but the publish workflow failed at Trivy's HIGH/CRITICAL gate (two `libpcre2-8-0` CVEs from a stale `debian:bookworm-slim` base). PR #50 fixes that root-cause style: builder → `messense/rust-musl-cross` (cross-compiles resql to static musl for both amd64 + arm64); runtime → `gcr.io/distroless/static-debian12:nonroot` (ca-certs + tzdata + /etc/passwd, no libc/libssl/libgcc — nothing that can carry a CVE); new `resql health` subcommand replaces curl in the HEALTHCHECK. **Trivy scan on the resulting image: 0 findings at any severity, from any source.** Post-merge the version needs to bump to `0.4.1-alpha` (auto-tag + auto-publish fires on merge); the vulnerable `:0.4.0-alpha` tags on both registries should also be deleted.
