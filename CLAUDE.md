...
- **In `Cargo.toml` on `dev`**: `0.4.1-alpha`. `v0.4.0-alpha` was tagged 2026-09-13 but publish failed at the Trivy HIGH gate (two `libpcre2-8-0` CVEs from a stale `debian:bookworm-slim` base). `0.4.1-alpha` fixes that root-cause style: builder → `messense/rust-musl-cross`; runtime → `gcr.io/distroless/static-debian12:nonroot` (ca-certs + tzdata + `/etc/passwd`, no libc/libssl/libgcc — nothing that can carry a CVE); new `resql health` subcommand replaces curl in HEALTHCHECK. **Trivy scan on the resulting image: 0 findings at any severity, from any source.** After the auto-tag + auto-publish fire on this bump PR merging, the vulnerable `:0.4.0-alpha` tags on both registries need to be deleted manually so nobody pulls them by accident.
